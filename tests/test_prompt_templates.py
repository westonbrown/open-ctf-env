...
from open_ctf.prompts.composer import (
    _NATIVE_SYSTEM_STUB,
    _TEMPLATES_DIR,
...
class TestGetCanonicalSystemPrompt:
    def test_returns_nonempty_string(self):
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
    ...
    def test_native_mode_returns_stub(self):
        with _env(OPEN_CTF_AGENT_MODE="native"):
            result = get_canonical_system_prompt()
        assert result == _NATIVE_SYSTEM_STUB

    def test_proxy_mode_returns_stub(self):
        with _env(OPEN_CTF_AGENT_MODE="proxy"):
            result = get_canonical_system_prompt()
        assert result == _NATIVE_SYSTEM_STUB
    ...
    def test_env_var_path_override(self, tmp_path: Path):
        ...
        with _env(
            ...
            OPEN_CTF_SYSTEM_PROMPT_PATH=str(custom),
        ):
            result = get_canonical_system_prompt()
    ...
    def test_explicit_path_overrides_env_var(self, tmp_path: Path):
        ...
        with _env(
            OPEN_CTF_AGENT_MODE="tool_calls",
            OPEN_CTF_SYSTEM_PROMPT_PATH=str(env_file),
        ):
...
class TestBuildRegistryUserPrompt:
    ...
    def test_web_category_hints(self):
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**_COMMON_KWARGS)
        assert "WEB CHALLENGE" in result
        assert "curl" in result

    def test_pwn_category_hints(self):
        kwargs = {**_COMMON_KWARGS, "category": "pwn"}
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**kwargs)
        assert "PWN CHALLENGE" in result
        assert "nc" in result or "netcat" in result

    def test_crypto_local_hints(self):
        kwargs = {
            **_COMMON_KWARGS,
            "category": "crypto",
        ...
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**kwargs)
        assert "CRYPTO CHALLENGE" in result
        assert "cipher" in result.lower() or "cryptographic" in result.lower()

    def test_crypto_remote_hints(self):
        kwargs = {
            **_COMMON_KWARGS,
            "category": "crypto",
            "target_url": "http://example.com:5555",
        }
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**kwargs)
        assert "CRYPTO CHALLENGE" in result
        assert "remote" in result.lower() or "TCP" in result

    def test_forensics_category_hints(self):
        kwargs = {**_COMMON_KWARGS, "category": "forensics"}
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**kwargs)
        assert "FORENSICS CHALLENGE" in result

    def test_misc_category_hints(self):
        kwargs = {**_COMMON_KWARGS, "category": "misc"}
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**kwargs)
        assert "MISC CHALLENGE" in result

    def test_unknown_category_no_crash(self):
        """An unknown category should produce a valid prompt without category hints."""
        kwargs = {**_COMMON_KWARGS, "category": "reverse_engineering"}
        with _env(OPEN_CTF_AGENT_MODE="tool_calls"):
            result = build_registry_user_prompt(**kwargs)
        assert "test-challenge-42" in result
        # Should NOT contain any category-specific section header
        for cat in ("WEB", "PWN", "CRYPTO", "FORENSICS", "MISC"):
            assert f"{cat} CHALLENGE" not in result

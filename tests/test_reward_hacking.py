...
Tests the CTFReward function against adversarial and realistic scenarios
...
def reward():
    ...
    return CTFReward(seed=42, noise_range=0.0)
...
class TestFlagPatternInThinking:
    ...
    def test_flag_in_think_no_submission(self, reward):
        ...
        comp = [{"role": "assistant", "content": "<think>I think the flag might be FLAG{test123}</think>"}]
    ...
    def test_flag_pattern_vs_exact_match(self, reward):
        """Exact flag match should score much higher than pattern."""
        pattern_only = [{"role": "assistant", "content": "Found FLAG{wrong_guess}"}]
        exact_match = [{"role": "assistant", "content": "Found FLAG{real_flag}"}]
        ...
        assert exact_score > pattern_score + 0.1, (
...
class TestLongHorizonCyberOps:
    ...
    def test_successful_10_step_exploit(self, reward):
        ...
        comp = completion([
            ...
            # ENUM
            shell("gobuster dir -u http://10.0.0.1 -w /usr/share/wordlists/common.txt"),
    ...
    def test_progressive_vs_random_ordering(self, reward):
        ...
        assert ordered_score > random_score, (
    ...
    def test_50_step_realistic_trace(self, reward):
        ...
        tools.extend([
            ...
            shell("sqlmap -u 'http://10.0.0.1/login' --data 'user=admin&pass=x' --batch"),
            shell("hydra -l admin -P /usr/share/wordlists/rockyou.txt http://10.0.0.1 http-post-form '/login:user=^USER^&pass=^PASS^:Invalid'"),
        ...
        tools.extend([
            python_code("# Found flag in user 10052\nprint('FLAG{idor_header_bypass}')"),
    ...
    def test_hallucinated_flag_submission(self, reward):
        ...
        assert scores[0] < no_submit_score, (
...
class TestComponentBreakdown:
    ...
    def test_exploration_early_novel_vs_late_novel(self, reward):
        ...
        early_novel = [
            ...
            {"name": "shell_command", "arguments": json.dumps({"command": "nmap again"})},
            {"name": "python_code", "arguments": json.dumps({"code": "exploit2()"})},
        ]
        late_novel = [
            {"name": "shell_command", "arguments": json.dumps({"command": "nmap target"})},
            {"name": "shell_command", "arguments": json.dumps({"command": "nmap again"})},
            {"name": "shell_command", "arguments": json.dumps({"command": "nmap more"})},
            {"name": "python_code", "arguments": json.dumps({"code": "exploit()"})},
            {"name": "read_file", "arguments": json.dumps({"path": "/etc/passwd"})},
        ]
...
class TestGRPOGradientSignal:
    ...
    def test_ranking_success_gt_partial_gt_fail(self, reward):
        ...
        assert s[0] > s[1] > s[2], f"Ranking broken: success={s[0]:.3f}, partial={s[1]:.3f}, fail={s[2]:.3f}"
    ...
    def test_ranking_efficient_gt_inefficient(self, reward):
        ...
        assert s[0] > s[1], f"Efficient ({s[0]:.3f}) should outscore inefficient ({s[1]:.3f})"
    ...
    def test_ranking_diverse_gt_repetitive(self, reward):
        ...
        assert s[0] > s[1], f"Diverse ({s[0]:.3f}) should outscore repetitive ({s[1]:.3f})"
    ...
    def test_ranking_structured_gt_unstructured(self, reward):
        ...
        assert s[0] > s[1], f"Structured ({s[0]:.3f}) should outscore unstructured ({s[1]:.3f})"
    ...
    def test_variance_across_batch(self, reward):
        ...
        completions = [
            ...
            completion([shell("nmap target"), shell("gobuster dir target"), python_code("exploit()")]),
            completion([shell("nmap target"), flag_found("FLAG{wrong}")]),
            completion([]),  # Empty
            completion([shell("nmap target"), shell("curl target"), python_code("exploit()"), flag_found("FLAG{win}")], text="FLAG{win}"),
        ...
        assert max(scores) - min(scores) > 0.3, f"Gap too small: {max(scores):.3f} - {min(scores):.3f}"
...
class TestOnlineModeEdgeCases:
    ...
    def test_empty_tool_response(self, reward):
        ...
        comp = [
            {"role": "assistant", "content": "", "tool_calls": [
                {"function": {"name": "shell_command", "arguments": json.dumps({"command": "nmap target"})}}
            ]},
    ...
    def test_multi_turn_tool_calling(self, reward):
        ...
        comp = [
            # Turn 1
            {"role": "assistant", "content": "<think>Let me scan first</think>", "tool_calls": [
                {"function": {"name": "shell_command", "arguments": json.dumps({"command": "nmap target"})}}
            ]},
            {"role": "tool", "content": "80/tcp open http"},
            # Turn 2
            {"role": "assistant", "content": "<think>Found HTTP, let me enumerate</think>", "tool_calls": [
                {"function": {"name": "shell_command", "arguments": json.dumps({"command": "curl -s target"})}}
            ]},
            {"role": "tool", "content": "<html>Login page</html>"},
            # Turn 3
            {"role": "assistant", "content": "<think>Found login, try SQLi</think>", "tool_calls": [
                {"function": {"name": "python_code", "arguments": json.dumps({"code": "exploit()"})}}
            ]},
            {"role": "tool", "content": "FLAG{sqli_success}"},
            # Turn 4: flag submission
            {"role": "assistant", "content": "Found flag: FLAG{sqli_success}", "tool_calls": [
                {"function": {"name": "flag_found", "arguments": json.dumps({"content": "FLAG{sqli_success}"})}}
            ]},
...
class TestScoreDistribution:
    ...
    def test_success_failure_gap(self, reward):
        ...
        assert gap > 0.3, f"Success-failure gap too small: {gap:.3f} (success={success_mean:.3f}, failure={failure_mean:.3f})"

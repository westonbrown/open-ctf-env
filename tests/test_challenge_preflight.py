...
def test_runtime_preflight_detects_registry_target_port_mismatch(tmp_path: Path, monkeypatch) -> None:
...
""".strip()
    ...
    monkeypatch.setattr(
        "open_ctf.challenges.preflight._probe_target_reachability",
...
""".strip()
    ...
    monkeypatch.setattr(
        "open_ctf.challenges.preflight._docker_ports_snapshot",
...
""".strip()
    ...
    assert any("Container port-binding verification unavailable" in msg for msg in report.warnings)


def test_runtime_preflight_raises_on_unreachable_target(tmp_path: Path, monkeypatch) -> None:

...
def test_tokenizer_drift_schema_injection():
    ...
    initial_prompt = [
        {"role": "system", "content": "You are a helpful CTF agent."}
    ]
    ...
    assert expected_hermes_instruction in sys_content, "Hermes format instruction drifted."
    assert expected_tool_def in sys_content, "Tool schema parameter formatting drifted."
    assert sys_content.startswith("You are a helpful CTF agent."), "System message prefix was lost."
    ...
    assert "You are a CTF agent with access to the following tools." in injected_no_sys[0]["content"]
...
def test_glm4_tool_call_format():
    ...
    env = OpenCTFTextEnv(target="http://localhost:8000", tool_call_format="glm4")

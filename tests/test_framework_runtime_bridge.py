"""Tests for src/open_ctf/agent/framework_runtime_bridge.py."""
...
BRIDGE = REPO_ROOT / "src" / "open_ctf" / "agent" / "framework_runtime_bridge.py"
...
""".strip()
    ...
    response = _run_bridge(
        ...
        env={
            ...
            "OPEN_CTF_AGENT_CMD": cmd,
            "OPEN_CTF_AGENT_CMD_TIMEOUT": "10",
        },
    )
...
def test_bridge_native_mode_preserves_protocol_passthrough_response() -> None:
    adapter = REPO_ROOT / "examples" / "adapters" / "boxpwnr_native_runtime_adapter.py"
    cmd = f"{shlex.quote(sys.executable)} {shlex.quote(str(adapter))}"
    response = _run_bridge(
        {

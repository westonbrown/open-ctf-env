"""Tests for TrajectoryLogger and CTFReward.compute_with_breakdown.
...
class TestTrajectoryLoggerBasic:
    ...
    def test_log_generation_with_full_data(self, tmp_path):
        ...
        tl.log_generation(
            ...
            tool_calls=[{"name": "shell_command", "args": {"command": "ls"}, "output": "flag.txt"}],
            reward_total=0.36,
...
class TestTrajectoryLoggerThreadSafety:
    ...
    def test_concurrent_scoreboard_updates(self, tmp_path):
        ...
        sb = tl.get_scoreboard()
        assert len(sb) == 5
        for cid, data in sb.items():
            assert data["attempts"] == 20


# ---------------------------------------------------------------------------
# CTFReward.compute_with_breakdown
# ---------------------------------------------------------------------------
...
class TestComputeWithBreakdown:
    ...
    def test_returns_tuple_list(self):
        reward = CTFReward(seed=42)
        completions = [
            _completion_with_tools(
                [_shell("nmap -sV target"), _shell("curl target/"), _shell("python3 exploit.py")],
    ...
    def test_breakdown_has_all_signals(self):
        ...
        completions = [
            _completion_with_tools(
                [_shell("nmap target"), _shell("curl target/"), _shell("python3 exploit.py")],
        ...
        expected_raw = {
            "flag", "efficiency", "progression", "exploration",
            "uniqueness", "format", "recovery", "cognitive", "hallucination",
        }
    ...
    def test_score_matches_call(self):
        ...
        reward = CTFReward(seed=42, noise_range=0.0)  # Zero noise for comparison
        completions = [
            _completion_with_tools(
        ...
        reward2 = CTFReward(seed=42, noise_range=0.0)
    ...
    def test_flag_found_breakdown(self):
        ...
        completions = [
            _completion_with_tools(
                [_shell("nmap target"), _shell("curl target/"), _tc("flag_found", {"content": "FLAG{test}"})],
                text=text,
            ),
        ]
        results = reward.compute_with_breakdown(
...
# ---------------------------------------------------------------------------
# Integration: TrajectoryLogger + CTFReward breakdown
# ---------------------------------------------------------------------------

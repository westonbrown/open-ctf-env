...
class TestRealDataComponentBreakdown:
    ...
    def test_score_distribution(self, reward, samples):
        ...
        scores = reward(completions, ground_truth_flag=gt_flags, optimal_steps=opt_steps)
    ...
    def test_component_breakdown(self, reward, samples):
        ...
        for i, s in enumerate(samples[:20]):
            ...
            flag_sc = reward._flag_score(text, gt, None)
            eff_sc = reward._efficiency_score(len(tool_calls), opt, flag_found=(flag_sc >= 1.0))
            ...
            print(f"       {len(tool_calls)} tools | {words} words | WPA={wpa:.0f} | "
        ...
        print(f"{'='*70}")
        print(f"  {'Component':<10} {'Mean':>8} {'Std':>8} {'Min':>8} {'Max':>8} {'NonZero':>10}")
        for name, vals in stats.items():
            if vals:
                ...
                print(f"  {name:<10} {mean:>8.4f} {std:>8.4f} {mn:>8.4f} {mx:>8.4f} "
                      f"{nz:>4d}/{len(vals)}")
    ...
    def test_no_reward_collapse(self, reward, samples):
        ...
        assert unique_scores > len(scores) * 0.3, \
    ...
    def test_cognitive_signal_on_real_data(self, reward, samples):
        ...
        print(f"  Cognitive: mean={mean_c:.3f}, std={std_c:.3f}, "

...
def test_default_candidates_include_reference_paths(monkeypatch) -> None:
    monkeypatch.delenv("OPEN_CTF_BOXPWNR_SRC", raising=False)
    monkeypatch.delenv("OPEN_CTF_DEFAULT_AGENT_SRC", raising=False)
...
def test_candidates_respect_env_and_dedupe(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("OPEN_CTF_BOXPWNR_SRC", str(custom))
    monkeypatch.setenv("OPEN_CTF_DEFAULT_AGENT_SRC", str(custom))

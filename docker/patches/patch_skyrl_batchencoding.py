...
def _find_call_end(content: str, call_start: int) -> int:
    ...
    while depth > 0 and idx < len(content):
        ...
        idx += 1
    if depth != 0:
        raise RuntimeError("Unbalanced parentheses while patching apply_chat_template call")
    return idx
...
def _ensure_helper(content: str) -> tuple[str, bool]:
    ...
    if dataclass_marker in content:
        return content.replace(dataclass_marker, "\n\n" + _HELPER_SNIPPET + dataclass_marker, 1), True
...
def patch_generator() -> int:
    ...
    if changes:
        ...
        print("   Patch (generator token normalization): already applied or patterns not found")
    return changes
...
def patch_client() -> bool:
    ...
    for client_path in CLIENT_PATHS:
        ...
        if "prompt_ids_obj = prompt_token_ids.get(\"input_ids\", prompt_token_ids)" in content:
            print(f"   Patch (remote client token normalization): already applied ({client_path})")
            continue
        ...
        if not replaced:
            print(f"   Patch (remote client token normalization): pattern not found ({client_path})")
            continue
...
def verify() -> None:
    ...
    if "def _to_token_ids(tokenized):" not in content:
        ...
        sys.exit(1)
    if "_to_token_ids(self.tokenizer.apply_chat_template(" not in content and "_to_token_ids(tokenizer.apply_chat_template(" not in content:
    ...
    for client_path in CLIENT_PATHS:
        ...
        if "prompt_ids_obj = prompt_token_ids.get(\"input_ids\", prompt_token_ids)" not in client_content:
            print(f"   Verification failed: client token normalization missing ({client_path})")
            sys.exit(1)

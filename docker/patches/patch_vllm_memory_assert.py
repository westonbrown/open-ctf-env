...
if FILEPATH is None or not FILEPATH.exists():
    print("   Patch (vllm memory assert): SKIP - vllm not installed or gpu_worker.py not found")
    sys.exit(0)
...
indent = lines[assert_start][:len(lines[assert_start]) - len(lines[assert_start].lstrip())]
...
print(f"PATCHED: Replaced assertion at lines {assert_start+1}-{assert_end} with warning+adjustment")

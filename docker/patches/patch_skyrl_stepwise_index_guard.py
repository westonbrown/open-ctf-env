...
CANDIDATES = [
    pathlib.Path("/usr/local/lib/python3.12/dist-packages/skyrl_train/generators/skyrl_gym_generator.py"),
]

# Dynamically find reference copies
for root in ["/workspace/open-ctf-env", "/workspace"]:
    ref = pathlib.Path(root) / "references/SkyRL-westonbrown/skyrl-train/skyrl_train/generators/skyrl_gym_generator.py"
    if ref.exists():
...
def main() -> int:
    ...
    for filepath in CANDIDATES:
        ...
        if count == 0:
            print(f"patch_skyrl_stepwise_index_guard: pattern not found in {filepath} (no changes)")
            continue

        filepath.write_text(updated)
        print(f"patch_skyrl_stepwise_index_guard: v2 applied to {filepath} ({count} replacements)")
        patched_count += 1

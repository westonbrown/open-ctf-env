...
FSDP_WORKER_PATH = pathlib.Path("/usr/local/lib/python3.12/dist-packages/skyrl_train/workers/fsdp/fsdp_worker.py")
...
def patch_fsdp_worker():
    ...
    if not match:
        print("   Patch 2 (CUDA IPC → HTTP /load_lora): SKIP - update_named_weights not found")
...
def patch_remote_inference_client():
    ...
    for client_path in REMOTE_CLIENT_CANDIDATES:
        ...
        if old_direct in content:
            ...
            print(f"   Patch 3 (remote client LoRA route): pattern not found ({client_path})")

    if not found:
        print("   Patch 3 (remote client LoRA route): SKIP - remote client file not found")
    elif not applied:
...
def patch_remote_inference_client_response_parsing():
    ...
    for client_path in REMOTE_CLIENT_CANDIDATES:
        ...
        if "body_text = await resp.text()" in content and "except Exception:" in content:
            print(f"   Patch 4 (remote client response parsing): already applied ({client_path})")
            continue
        ...
        if not replaced:
            print(f"   Patch 4 (remote client response parsing): pattern not found ({client_path})")
            continue
    ...
    if not found:
        print("   Patch 4 (remote client response parsing): SKIP - remote client file not found")
    elif not applied:

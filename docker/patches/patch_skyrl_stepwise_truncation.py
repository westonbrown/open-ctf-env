...
if "skip truncation for step_wise" in content or "step_wise_trajectories" in content.split("max_trajectories")[1][:200] if "max_trajectories" in content else False:
...
if old not in content:
    ...
    if re.search(pattern, content):
        print("Found the pattern but exact string mismatch. Attempting line-level fix...")
        ...
        for i, line in enumerate(lines):
            if "if len(final_uids) > max_trajectories:" in line:
                ...
                lines.insert(i, f"{indent}_is_step_wise = bool(final_output.get('is_last_step'))")
                lines[i+1] = f"{indent}if not _is_step_wise and len(final_uids) > max_trajectories:"
                patched = True
                break
        if patched:
            content = "\n".join(lines)

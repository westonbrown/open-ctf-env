...
def _probe_target_reachability(target: str, timeout_seconds: float) -> tuple[bool, str, str]:
    ...
    if scheme in {"http", "https"}:
        ...
        for method in ("HEAD", "GET"):
            ...
            try:
                conn.request(method, path, headers={"User-Agent": "open-ctf-preflight/1.0"})
    ...
    *,
    ...
    if strict_container_check:
        ...
        if needs_local_docker_check:
            docker_ports_text, docker_snapshot_error = _docker_ports_snapshot(timeout_seconds)
    ...
    for cid in selected:
        ...
        if info.infra_type == "docker":
            ...
            elif info.port is not None and target_port is not None and info.port != target_port:
                report.errors.append(
        ...
        if require_reachable:
            reachable, check_mode, detail = _probe_target_reachability(target, timeout_seconds)
            if not reachable:
                report.errors.append(
...
def resolve_challenge_id_or_raise(registry: ChallengeRegistry, challenge_id: str) -> str:

...
def _convert_online_rl_data(
    ...
    with jsonlines.open(data_path) as reader, jsonlines.open(output_path, "w") as writer:
        for sample in reader:
            ...
            for msg in messages:
                if msg.get("role") == "user":
                    urls = re.findall(r"(?:https?|file)://[^\s)]+", msg.get("content", ""))
                    if urls:
                        target = urls[0]
                        break
            if not target:
    ...
    if flag_mismatch_counts:
        top = sorted(flag_mismatch_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        msg = (
        ...
        "trainer": {
            ...
            "algorithm": {
                ...
                "off_policy_correction": {
                    ...
                    "sequence_mask_metric": off_policy_correction.get("sequence_mask_metric"),
                    "geo_mask_high": float(off_policy_correction.get("geo_mask_high", 1.01)),
                    "geo_mask_low": float(off_policy_correction.get("geo_mask_low", 0.99)),
                    "product_mask_high": float(off_policy_correction.get("product_mask_high", 2.0)),
                    "product_mask_low": float(off_policy_correction.get("product_mask_low", 0.5)),
                ...
                with open(summary_path) as f:
                    for line in f:
                        ...
                        if not line:
                            continue
                        try:
                            latest_step = int(json.loads(line).get("global_step", latest_step))

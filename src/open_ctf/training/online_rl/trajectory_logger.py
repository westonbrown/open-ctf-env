...
class TrajectoryLogger:
    ...
    def _update_step_aggregate_locked(
        ...
        *,
        ...
        if isinstance(reward_breakdown, dict):
            for key, value in reward_breakdown.items():
                try:
                    ...
                    continue
                state["signal_sums"][key] = float(state["signal_sums"].get(key, 0.0)) + v
        ...
        state["status_counts"][status_value] = int(state["status_counts"].get(status_value, 0)) + 1
        ...
        if isinstance(timing, dict):
            for key, value in timing.items():
                try:
                    ...
                    continue
                state["timing_sums"][key] = float(state["timing_sums"].get(key, 0.0)) + v
                state["timing_counts"][key] = int(state["timing_counts"].get(key, 0)) + 1
    ...
    def _build_step_summary_locked(self, global_step: int, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        min_reward = float(state["min_reward"]) if state["min_reward"] is not None else 0.0
        max_reward = float(state["max_reward"]) if state["max_reward"] is not None else 0.0
        if total > 1:
            variance = (
                float(state["reward_sq_sum"]) - (float(state["reward_sum"]) ** 2) / total
            ) / (total - 1)
    ...
    def _emit_step_summary_to_tensorboard(self, summary: Dict[str, Any]) -> None:
        ...
        self._tb_writer.add_scalar("reward/avg", float(summary.get("avg_reward", 0.0)), step)
        self._tb_writer.add_scalar("reward/min", float(summary.get("min_reward", 0.0)), step)
        self._tb_writer.add_scalar("reward/max", float(summary.get("max_reward", 0.0)), step)
        self._tb_writer.add_scalar("reward/std", float(summary.get("reward_std", 0.0)), step)
        self._tb_writer.add_scalar("flag/found_rate", float(summary.get("flag_found_rate", 0.0)), step)
        self._tb_writer.add_scalar("tools/avg_calls", float(summary.get("avg_tool_calls", 0.0)), step)
        ...
        if straggler_count > 0:
            self._tb_writer.add_scalar(
                "latency/straggler_count", straggler_count, step
            )
        ...
        **extra: Any,
        ...
        with self._lock:
            ...
            summary = self._update_step_aggregate_locked(
                ...
                timing=entry.get("timing") if isinstance(entry.get("timing"), dict) else None,
            )
    ...
    def flush_scoreboard_to_tensorboard(self, global_step: int = 0) -> None:
        ...
        with self._lock:
            for cid, data in self._scoreboard.items():
                ...
                self._tb_writer.add_scalar(f"challenge/{safe_cid}/solve_rate", solve_rate, global_step)
                self._tb_writer.add_scalar(f"challenge/{safe_cid}/avg_reward", avg_r, global_step)

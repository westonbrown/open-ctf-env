...
class SimulatedEnvironmentExecutor(BaseExecutor):
    ...
    def step(self, tool_name: str, arguments: dict, generation_id: Optional[str] = None) -> dict:
        ...
        if tool_name in ("submit_flag", "flag_found"):
            submitted = (arguments.get("content", "") or arguments.get("flag", "") or arguments.get("value", "")).strip()
    ...
    def _mock_shell(self, args: Dict[str, Any]) -> tuple[str, str, int]:
        ...
        if cmd.strip() == "whoami" or "whoami" in cmd.split("&&")[0]:
            return "root\n", "", 0
        if "uname" in cmd:
            return "Linux target 5.15.0-88-generic #98-Ubuntu SMP x86_64 GNU/Linux\n", "", 0
        ...
        if any(cmd.strip().startswith(c) for c in ("cd ", "mkdir ", "chmod ", "chown ", "touch ", "echo ")):
    ...
    def _check_credential_command(self, cmd: str) -> Optional[str]:
        ...
        for fnode in self._current_manifest.files.values():
            ...
            for match in re.findall(r"(?:pass|password|secret|key|token)\s*[=:]\s*['\"]?(\S{4,})['\"]?", fnode.content, re.IGNORECASE):
    ...
    def _mock_ls(self, cmd: str) -> tuple[str, str, int]:
        ...
        if detailed:
            ...
            for name, perms, owner, size, is_dir in sorted(entries):
                ...
                lines.append(f"{prefix}{perms} 1 {owner:<8} {owner:<8} {size:>6} Mar  1 10:00 {name}")
            return "\n".join(lines) + "\n", "", 0
        else:
    ...
    def _mock_curl(self, cmd: str) -> tuple[str, str, int]:
        ...
        for host in self._current_manifest.hosts.values():
            if host.ip in url or host.hostname in url:
                ...
                http_services = [s for s in host.services if s.name in ("http", "https", "http-proxy")]
                if http_services:
                    srv = http_services[0]
                    return (
                ...
                return f"Connection to {host.hostname} ({host.ip}) established.\nServices: {services}\n", "", 0
    ...
    def _mock_shell_grep(self, cmd: str) -> tuple[str, str, int]:
        ...
        match = re.search(r"(?:grep|egrep|fgrep)\s+(?:-[a-zA-Z]*\s+)*['\"]?([^'\"]+?)['\"]?\s+(.+)$", cmd.strip())
        if not match:
        ...
        for fpath, fnode in self._current_manifest.files.items():
            if target != "." and target not in fpath and os.path.basename(fpath) != target:
                continue
            for i, line in enumerate(fnode.content.split("\n"), 1):
                if pat.search(line):
                    results.append(f"{fpath}:{i}:{line.strip()}")
    ...
    def _mock_process_info(self, cmd: str) -> tuple[str, str, int]:
        if cmd.strip().startswith("ps"):
            ...
            for host in self._current_manifest.hosts.values():
                for srv in host.services:
                    lines.append(f"root       {100+srv.port:>5}  0.1  0.2  45000  8192 ?        Sl   10:00   0:05 {srv.name} (port {srv.port})")
            return "\n".join(lines) + "\n", "", 0
        ...
        if cmd.strip().startswith(("netstat", "ss")):
            ...
            for host in self._current_manifest.hosts.values():
                for srv in host.services:
                    lines.append(f"tcp    0      0 0.0.0.0:{srv.port}            0.0.0.0:*               LISTEN")
            return "\n".join(lines) + "\n", "", 0

...
class WorldManifest:
    ...
    def from_yaml(cls, path: str) -> "WorldManifest":
        ...
        manifest = cls(
            ...
            enforce_topology=data.get("world_state_dynamics", {}).get("enforce_topology", False),
            namespaces=data.get("world_state_dynamics", {}).get("namespaces", []),
            synthetic_faults=data.get("world_state_dynamics", {}).get("synthetic_faults", {}),
        )
    ...
    def clone(self) -> "WorldManifest":
        ...
        if "FLAG{" in old_flag:
            ...
            # Sub into files
            for file_path, file_node in cloned.files.items():
                if file_node.content and old_flag in file_node.content:
                    file_node.content = file_node.content.replace(old_flag, cloned.ground_truth_flag)
            ...
            for tool_name, responses in cloned.tool_responses.items():
                for cmd, output in responses.items():
                    if output and old_flag in output:
                        cloned.tool_responses[tool_name][cmd] = output.replace(old_flag, cloned.ground_truth_flag)

...
def preprocess_sample(sample: dict) -> tuple[dict | None, dict]:
    ...
    if not has_flag_call and ground_truth:
        ...
        messages.append(_make_verification(
...
def process_file(input_path: Path, output_path: Path | None) -> dict:
    ...
    try:
        for line in open(input_path):
            ...
            for key in ("html_fixed", "think_fixed", "echo_stripped",
                        "noncanonical_stripped", "verification_injected",
                        "verification_replaced", "terminal_flag_added"):

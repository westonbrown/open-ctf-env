...
def build_messages(
    ...
    return [
        {
            "role": "system",
            "content": get_canonical_system_prompt()
        },
        {
            "role": "user",
...
def main():
    ...
    parser.add_argument("--registry", type=str, required=True, help="Path to cybench.yaml")
    parser.add_argument("--target-map", type=str, required=True, help="Path to cybench_target_map.json")
    parser.add_argument("--output", type=str, required=True, help="Output JSONL path")
    parser.add_argument("--probe-targets", action="store_true", help="Probe TCP/HTTP to ensure targets are reachable")
    parser.add_argument("--strict-target-probe", action="store_true", help="Fail entirely if any target is unreachable")
    parser.add_argument("--difficulty-max", type=str, help="Max difficulty to include (very_easy, easy, medium, hard)")
    ...
    max_diff_val = diff_order.get(args.difficulty_max, 99) if args.difficulty_max else 99
    ...
    if "domains" in target_map:
        for domain, cmap in target_map["domains"].items():
    ...
    for chall in challenges:
        ...
        if args.probe_targets:
            if not _is_reachable(target_url, category):

...
def cmd_compare(args: argparse.Namespace) -> None:
    ...
    from open_ctf.eval.evaluator import compare_reports
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Open CTF Model Evaluation",
    ...
    run_parser.add_argument("--output", "-o", required=True, help="Output directory for reports")
    run_parser.add_argument(
        "--challenges",
        default=str(DEFAULT_CHALLENGES),
        help="Path to challenges YAML",
    )
    run_parser.add_argument("--platform", default="xbow", help="Default platform (default: xbow)")
    run_parser.add_argument("--strategy", default="chat_tools", choices=["chat", "chat_tools"])
    run_parser.add_argument("--max-turns", type=int, default=50)
    run_parser.add_argument("--max-time", type=int, default=30, help="Max time per challenge (min)")
    ...
    cmp_parser.add_argument("--base", required=True, help="Path to base model eval_report.json")
    cmp_parser.add_argument("--tuned", required=True, help="Path to fine-tuned eval_report.json")

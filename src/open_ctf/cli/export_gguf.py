...
Pipeline:
  1. Merge LoRA adapter into base model weights (Unsloth)
...
def convert_to_gguf(model_dir: str, output_path: str, llama_cpp_dir: Path) -> str:
    ...
    f16_path = output_path.replace(".gguf", "-f16.gguf") if output_path.endswith(".gguf") else output_path + "-f16.gguf"
...
def quantize_gguf(f16_path: str, output_path: str, quant_type: str, llama_cpp_dir: Path) -> str:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="HuggingFace base model ID (e.g. unsloth/GLM-4.7-Flash)",
    )
    parser.add_argument(
    ...
    with tempfile.TemporaryDirectory(prefix="octf_merge_") as merge_dir:
        merged_dir = merge_dir if not args.keep_intermediates else str(output_path.parent / "merged")
        ...
        if args.quant.lower() != "none":
            ...
            if not args.keep_intermediates and Path(f16_path).exists() and f16_path != str(output_path):

...
def generate_html(output_dir: str) -> str:
    ...
    for cid, data in sorted(scoreboard.items()):
        ...
        bar_color = "var(--green)" if solve_pct > 50 else "var(--yellow)" if solve_pct > 0 else "var(--red)"
        scoreboard_rows += f"""
        <tr>
    ...
    overall_flag_rate = round(total_flags / total_gens * 100, 1) if total_gens > 0 else 0
...
<title>open-ctf Trajectory Browser</title>
<style>
:root {{
...
<h1>open-ctf Trajectory Browser</h1>

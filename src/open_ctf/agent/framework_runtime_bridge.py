#!/usr/bin/env python3
"""Generic BYO-agent runtime bridge for OpenCTF online RL.
...
The bridge always speaks OpenCTF runtime protocol v1.0.
"""
...
from open_ctf.agent.runtime_protocol import (
...
def _request_settings(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mode = _normalize_mode(
        ...
        or _env("OPEN_CTF_AGENT_MODE", MODE_TOOL_CALLS)
    )
    framework = str(
        state.get("agent_framework")
        or _env("OPEN_CTF_AGENT_FRAMEWORK", DEFAULT_FRAMEWORK)
    ).strip() or DEFAULT_FRAMEWORK
    cmd = str(
        state.get("agent_cmd")
        or _env("OPEN_CTF_AGENT_CMD", "")
    ).strip()
    ...
    workdir = str(
        state.get("agent_cmd_workdir")
        or _env("OPEN_CTF_AGENT_CMD_WORKDIR", "")
    ).strip()
    timeout_raw = (
        state.get("agent_timeout_seconds")
        or _env("OPEN_CTF_AGENT_CMD_TIMEOUT", "120")
    )
    try:
...
def _run_external_framework(
    ...
    if not cmd:
        raise RuntimeError(
            "native mode requires OPEN_CTF_AGENT_CMD (or runtime_state.agent_cmd)"
        )
    ...
    if isinstance(raw, dict) and raw.get("protocol_version"):
        if "passthrough_response" in raw and isinstance(raw["passthrough_response"], dict):
        ...
        elif "tool_calls_response" in raw and isinstance(raw["tool_calls_response"], dict):
    ...
    episode_done = bool(raw.get("episode_done", False)) if isinstance(raw, dict) else False
    all_text_append = str(raw.get("all_text_append", "")) if isinstance(raw, dict) and raw.get("all_text_append") else None
...
def main() -> int:
    ...
    if not raw.strip():
        info = {
            ...
            "framework": _env("OPEN_CTF_AGENT_FRAMEWORK", DEFAULT_FRAMEWORK),
            "mode": _normalize_mode(_env("OPEN_CTF_AGENT_MODE", MODE_TOOL_CALLS)),

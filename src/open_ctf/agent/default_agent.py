...
class DefaultStepAgent:
    ...
    def _normalize_action_text(self, action: Any) -> str:
        ...
        if isinstance(action, (list, tuple)) and action and all(
            isinstance(x, int) for x in action
        ):
    ...
    def _run_external_runtime(self, action: str) -> tuple[Optional[Dict[str, Any]], float]:
        ...
        if proc.returncode != 0:
            ...
            msg = (
                f"External runtime exited {proc.returncode}: {self.runtime_cmd}"
        ...
        **kwargs: Any,
        ...
        if kwargs.get("tool_call_format") is not None:
            self._tool_call_format = (
                str(kwargs.get("tool_call_format", "")).strip() or self._tool_call_format
            )
    ...
    def step(self, action: str) -> StepResult:
        ...
        if isinstance(runtime_payload, dict):
            ...
            if self.runtime_passthrough or bool(runtime_payload.get("passthrough")):
                ...
                done = bool(runtime_payload.get("done", False)) or self.turns >= self.max_steps
                ...
                info = {
                    **runtime_info,
                    "tool_calls": int(runtime_info.get("tool_calls", len(runtime_tool_calls))),
                    "step": self.turns,
        ...
        if not tool_calls:
            ...
            if runtime_info.get("rollout_status"):
                candidate = normalize_rollout_status(runtime_info.get("rollout_status"), default=RolloutStatus.NO_TOOL_CALL)
            ...
            if done:
                return self._apply_resilient_retry(StepResult(
                    observations=[],
                    done=True,
                    info=info_payload,
                ))
        ...
        if done:
            if self.episode_done:
                ...
                status = RolloutStatus.MAX_TURN_ABORT.value
        if runtime_info.get("rollout_status"):
            candidate = normalize_rollout_status(runtime_info.get("rollout_status"), default=RolloutStatus.OK)
            status = _upgrade_status(status, candidate)
        timing = {
            "parse_s": parse_seconds,
            "execute_s": execute_seconds,
    ...
    def _apply_resilient_retry(self, result: StepResult) -> StepResult:
        ...
        status = normalize_rollout_status(
            (result.info or {}).get("rollout_status", "")
        )

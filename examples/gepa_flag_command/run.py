...
Demonstrates Stage 3 of the Open CTF training pipeline: evolving a system
...
def write_challenge_jsonl(target_port: int) -> Path:
    ...
    challenge["messages"][0]["content"] = (
        challenge["messages"][0]["content"].format(port=target_port)
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Model identifier for dspy.LM (e.g. openai/qwen35-27b)",
    )
    parser.add_argument(
        "--vllm-port",
        type=int,
    ...
    if not check_target(args.target_port):
        logger.error(
            "Start challenge: open-ctf-challenges setup "
            "--challenge '[Very Easy] Flag Command'"
        )
        ok = False

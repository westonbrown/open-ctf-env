...
#
# Usage:
#   bash examples/smoke_test_2ch.sh --model outputs/sft-nanbeige3b-merged
#
# Options:
...
resolve_project_dir() {
    if [[ -n "${OPEN_CTF_PROJECT_DIR:-}" && -d "${OPEN_CTF_PROJECT_DIR}/src/open_ctf" ]]; then
        echo "${OPEN_CTF_PROJECT_DIR}"
        return 0
    fi
    ...
    if [[ -d "/workspace/open-ctf-env/src/open_ctf" ]]; then
        echo "/workspace/open-ctf-env"
        return 0
    fi
...
CONFIG="$PROJECT_DIR/configs/training/training_smoke_2ch.yaml"
...
resolve_python_bin() {
    local explicit="${OPEN_CTF_PYTHON:-}"
    ...
    local candidates=(
        ...
        "/workspace/.venv-grpo-iso/bin/python"
        "/workspace/.venv-grpo/bin/python"
        "python3"
    )
    local c
...
if [[ -z "$MODEL_PATH" ]]; then
    ...
    echo "Usage: bash examples/smoke_test_2ch.sh --model outputs/sft-nanbeige3b-merged"
    exit 1
fi
...
if [[ -n "$CUSTOM_TARGET_MAP" ]]; then
    ...
    if [[ -d "/workspace/cybench" ]]; then
        BENCHMARK_ROOT="/workspace/cybench"
    fi
    "$PYTHON_BIN" "$PROJECT_DIR/src/open_ctf/cli/generate_target_map.py" \
        --registry "$REGISTRY" \
...
"$PYTHON_BIN" "$PROJECT_DIR/src/open_ctf/cli/generate_online_rl.py" \
    --registry "$REGISTRY" \
...
" 2>&1 || {
        ...
        echo "  Start them with: open-ctf-challenges setup --registry $REGISTRY"
...
if [[ "$LOCAL_INFERENCE" == "true" ]]; then
    ...
    if curl -s "${VLLM_URL}/health" > /dev/null 2>&1; then
        ...
        CUDA_VISIBLE_DEVICES="$VLLM_CUDA_DEVICES" VLLM_ENABLE_V1_MULTIPROCESSING=0 nohup "$PYTHON_BIN" -m open_ctf.training.skyrl_vllm_server \
            --model "$MODEL_PATH" \
...
export OPEN_CTF_TARGET_MAP_PATH="$TARGET_MAP"
export OPEN_CTF_TARGET_MAP_STRICT=1
if [[ "$STEP_DEBUG" == "true" ]]; then
    export OPEN_CTF_STEP_DEBUG=1
fi
...
if __name__ == "__main__":
    ...
    if rl_key:
        ...
        config[rl_key]["agent_class"] = "open_ctf.agent.default_agent.DefaultStepAgent"
        ...
        config[rl_key]["agent_kwargs"].setdefault("runtime_cmd", "python src/open_ctf/agent/framework_runtime_bridge.py")
        ...
        config[rl_key]["agent_kwargs"]["runtime_env"].setdefault("OPEN_CTF_AGENT_FRAMEWORK", "boxpwnr_langgraph")
        config[rl_key]["agent_kwargs"]["runtime_env"].setdefault("OPEN_CTF_AGENT_MODE", "tool_calls")
        if "$NATIVE_BOXPWNR".lower() == "true":
            config[rl_key]["agent_kwargs"]["runtime_env"]["OPEN_CTF_AGENT_MODE"] = "native"
            config[rl_key]["agent_kwargs"]["runtime_env"]["OPEN_CTF_AGENT_CMD"] = (
                "python examples/adapters/langgraph_native_runtime_adapter.py"
            )
            config[rl_key]["agent_kwargs"]["runtime_env"].setdefault(
                "OPEN_CTF_AGENT_CMD_TIMEOUT", "20"
            )
            ...
            config[rl_key]["agent_kwargs"]["runtime_env"]["OPEN_CTF_TOOL_CALL_FORMAT"] = selected_format
            ...
            config[rl_key]["agent_kwargs"]["runtime_env"]["OPEN_CTF_AGENT_MODE"] = "proxy"
            config[rl_key]["agent_class"] = "open_ctf.agent.proxy_step_agent.ProxyStepAgent"
            config[rl_key]["agent_kwargs"]["agent_cmd"] = "python src/open_ctf/cli/run_proxy.py"
...
cd "$PROJECT_DIR"
CUDA_VISIBLE_DEVICES="$TRAIN_CUDA_DEVICES" OPEN_CTF_STEP_DEBUG="${OPEN_CTF_STEP_DEBUG:-0}" "$PYTHON_BIN" "$OUTPUT_DIR/run_training.py" 2>&1 | tee "$OUTPUT_DIR/training.log"
TRAIN_EXIT=$?

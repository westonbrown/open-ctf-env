...
### Active RunPod TODO (No Monkey Patch)

- [x] Verify root issue in upstream SkyRL refs: remote inference engines reject `LoraLoadRequest` for LoRA sync.
- [x] Remove OpenCTF runtime monkey patch path from GRPO training entrypoint.
- [x] Add explicit topology resolver in `grpo.py` to prevent unsupported remote+LoRA combinations.
- [x] Make ChallengeManager benchmark-root discovery robust for nested benchmark layouts (BoxPwnr/CyBench style).
- [x] Make SFT curation registry-aware via canonical `resolve_id()` (drop unresolved/non-benchmark rows safely).
- [x] Re-apply deterministic SkyRL site-package patch set on RunPod after sync (`import re` + BatchEncoding call-site rewrites).
- [x] Patch SkyRL `RemoteInferenceClient` LoRA routing: `LoraLoadRequest` now targets `/v1/load_lora_adapter` (object-attr safe, no runtime monkey patch).
- [x] Root-cause latest GRPO 400: `token_ids=['input_ids','attention_mask']` from naive `list(BatchEncoding)` coercion.
- [x] Harden BatchEncoding fix: `_to_token_ids(...)` helper + remote prompt-id normalization (handles mapping/tensor/nested forms).
- [x] Harden remote control-plane parsing: tolerate non-JSON HTTP 2xx bodies in `_call_all_servers`.
- [x] Fix SkyRL custom chat-template + response-logprobs incompatibility by forcing `sampling_params.logprobs=None` when `chat_template` is set (plus regression test coverage).
- [x] Add deterministic inference-backend guard: force legacy SkyRL inference for `*TextConfig` models (for example Qwen3.5 text wrapper) to avoid vLLM new-inference type mismatch.
- [x] Root-cause GRPO model handoff mismatch: use merged vLLM-compatible checkpoint (`*_merged_vllm`, `model_type=qwen3_5`) instead of text-wrapper checkpoint (`model_type=qwen3_5_text`) for SkyRL+vLLM runtime.
- [x] Harden `patch_vllm_serving_api.py` so patching SkyRL `vllm_engine.py` is syntax-safe/idempotent across upstream layouts (fixes malformed `else` insertion failure mode on new instances).
- [x] Add GRPO model-path resolver: auto-switch `*_text` checkpoints to sibling `*_vllm` path when present (prevents repeated handoff mismatch on new runs).
- [ ] Run clean online GRPO smoke on RunPod using local non-colocated engines (`run_engines_locally=true`, `colocate_all=false`) and confirm first reward-bearing step.
- [ ] Validate weight updates, tool execution loop, and reward logging in GRPO runtime logs.
- [ ] Launch longer GRPO run on CyBench data after smoke pass.
- [x] Launch dual-B200 Qwen3.5-27B production SFT (48K-57K context candidates, 24h cap) on curated CyBench-focused data. (Started: `runpod_sft_then_vllm_prod_20260225_015359.log`)
- [ ] Merge adapter and stage next GRPO handoff model, then restart vLLM on dedicated GPU for RL.

### Next: DGX Deployment & Live Validation

#### Phase 1 — Transfer & Install
- [x] **Rsync to DGX Spark**: `rsync -avz open-ctf-env/ abrown@100.91.175.48:/home/abrown/open-ctf-env/`
- [x] **Docker build**: Container `open-ctf-test` running on DGX (based on `vllm-node-tf5`)
- [x] **Dep install**: skyrl-gym, skyrl-train, ray, vllm all resolve in container
- [x] **CyBench setup**: 40 challenges registered, 1/3 docker challenges reachable

#### Phase 2 — SFT Smoke Test
- [x] **SFT run**: Nanbeige4.1-3B SFT completed, merged model at `outputs/sft-nanbeige3b-merged`
- [x] **Merge**: LoRA adapter merged into full model
- [ ] **Sanity check**: Quick inference on merged model (can it generate valid tool calls?)

#### Phase 3 — Online GRPO with Live CyBench
- [x] **Fix Issue #24/28**: vLLM 0.16 V0 engine fully removed; use non-colocated topology on constrained GPUs
- [x] **Fix Issue #31**: `additional_kwargs` removed from SamplingParams (vLLM 0.16 compat)
- [x] **GRPO smoke test (3 steps)**: DefaultStepAgent + SubprocessExecutor + CTFReward (2026-02-24)
- [x] **Reward signal confirmed**: Step 1 avg_final_rewards=0.25 (1/4 generations found flag)
- [x] **Checkpoint saved**: `global_step_4` with LoRA adapter at `/tmp/grpo_smoke_ext/`
- [ ] **Full GRPO run**: `open-ctf-train grpo --model outputs/sft-merged --data data/grpo.jsonl --output outputs/grpo` (50-100 steps)
- [ ] **Monitor**: KL divergence, grad norms, reward trend over steps
...
Tests 1-5 (pre-GPU, no vLLM needed):
  ...
  ✅ CyBench registry loads (40 challenges, 15 docker)
...
|-------|--------------|---------------------|
| `patch_skyrl_bf16_policy_init.py` | `bf16=False` hardcoded → 30B fp32 = 120GB OOM | **YES** |
| `patch_skyrl_weight_sync.py` | NCCL deadlock + CUDA IPC → skip, use file-based `/load_lora` | **YES** (server mode) |
...
#### MoE-Specific Config (configs/skyrl/glm47_flash.yaml)
...
| **Disk** | 4TB NVMe |
| **Network** | Tailscale: `100.91.175.48` / `spark-0e3a.tailb91e18.ts.net`, LAN: `192.168.4.45` |
| **SSH** | `ssh abrown@100.91.175.48` |
...
docker run -d --name open-ctf-test --gpus all \
    --shm-size=64g \
    -v /home/abrown/open-ctf-env:/workspace/open-ctf-env \
    -p 8001:8001 \
    vllm-node-tf5 \
    sleep infinity
```
...
### RunPod B200 Deployment (2026-02-24)
...
- **Config files**: `configs/skyrl/qwen35_27b.yaml`, `configs/llamafactory/qwen35_27b.yaml`

## Workflow: Always Use Sub-Agents / Agent Teams

When working on this project, **always spawn sub-agents or agent teams to parallelize work**. This is a complex multi-system project (vLLM + SkyRL + LlamaFactory + Docker + remote pods) and tasks are often independent.

**Examples of parallel work:**
- Code changes locally + SSH operations on RunPod simultaneously
- vLLM upgrade + model download + code rsync in parallel
- Config creation + parser implementation + compatibility research concurrently
- SFT training monitoring + GRPO config prep at the same time

**Rules:**
1. If a task has 3+ independent sub-tasks, spawn sub-agents
2. Use `run_in_background: true` for long-running SSH operations
3. Create TaskList items to track progress across agents
4. Don't duplicate work — if an agent is investigating something, don't also investigate it

...
The output JSON can be consumed by GRPO via:
  OPEN_CTF_TARGET_MAP_PATH=/path/to/map.json
or:
  grpo.target_map_path: /path/to/map.json
"""
...
def _resolve_from_path_matches(
    ...
    for record in matches:
        if not service_hint_norm:
            continue
        if service_hint_norm not in {_normalize(record.service), _normalize(record.name)}:
            continue
        siblings = [m for m in matches if m.project == record.project and m.ports]
        if not siblings:
            continue
        sibling = sorted(siblings, key=lambda r: _pick_binding(r.ports, container_port_hint).host_port)[0]
...
def _resolve_fallback(
    ...
    for record in containers:
        ...
        if _normalize(info.name) and _normalize(info.name) in record_norm:
            score += 90
        if service_hint and _normalize(service_hint) in {record_norm, service_norm, project_norm}:
            score += 140
        if container_port_hint and any(p.container_port == container_port_hint for p in record.ports):
            score += 40
        overlap = len(query_tokens & (_iter_tokens(record.name) | _iter_tokens(record.project)))
        if overlap:
            score += overlap * 25
        if score > 0:
...
def build_live_target_map(
    ...
    for info in docker_challenges:
        ...
        matches = [c for c in containers if _path_matches(info.path_hint, benchmark_root, c)]

...
def grade_manifest_adherence(trace: dict, manifest: WorldManifest) -> dict:
    ...
    svc_refs = sum(1 for m in messages if any(sn in str(m.get("content", "")) for sn in service_names))
    ...
    for fnode in manifest.files.values():
        ...
        for match in re.findall(r"(?:pass|password|secret|key)\s*[=:]\s*['\"]?(\S{4,})['\"]?", fnode.content, re.IGNORECASE):
        ...
        if m.get("role") in ("assistant",):
            for tc in m.get("tool_calls", []):
                ...
                for cred in creds_in_files:
                    if cred in args_str:
                        creds_used.add(cred)
    scores["credential_usage"] = len(creds_used) / max(1, len(creds_in_files)) if creds_in_files else 0.5
    if creds_used:
    ...
    think_count = sum(1 for m in assistant_msgs if "<think>" in str(m.get("content", "")))
...
def main():
    ...
    parser.add_argument("--manifests", default="default", help="'default', 'all', or comma-separated YAML filenames")
    ...
    if args.manifests == "all":
        ...
        manifest_paths = [os.path.join(MANIFEST_DIR, m.strip()) for m in args.manifests.split(",")]
    ...
    if len(results) > 1:
        ...
        avg_adherence = sum(r["adherence"]["scores"]["overall"] for r in results) / len(results)
        ...
        for r in results:
            ...
            print(f"    [{status}] {r['challenge']:<40} adherence={r['adherence']['scores']['overall']:.3f}  tools={r['tool_call_count']}  turns={r['total_turns']}")

    # Dump JSON
    print("\n--- JSON ---")
    print(json.dumps(results if len(results) > 1 else results[0] if results else {}, indent=2, default=str))

...
CURATED_DATA_PATH="${CURATED_DATA_PATH:-${PROJECT_ROOT}/data/sft_cybench_platform_curated.jsonl}"
...
mkdir -p "${OUTPUT_ROOT}" /workspace/outputs
...
pkill -f "vllm serve|open_ctf.training.skyrl_vllm_server|vllm.entrypoints.openai.api_server" >/dev/null 2>&1 || true
...
for CTX in ${CONTEXT_CANDIDATES}; do
    ...
    LOG_PATH="/workspace/outputs/sft_qwen35_ctx${CTX}_${TIMESTAMP}.log"
...
if [[ "${MERGE_AFTER_SFT}" == "1" ]]; then
    ...
    CUDA_VISIBLE_DEVICES=0,1 \
    PYTHONPATH="${PROJECT_ROOT}/src:${PYTHONPATH:-}" \
    python3 -m open_ctf.cli.train --config "${TRAINING_CONFIG}" merge \
...
VLLM_LOG="/workspace/outputs/vllm_after_sft_${TIMESTAMP}.log"

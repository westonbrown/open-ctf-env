...
def _load_target_rows(path: Path) -> list[dict]:
    ...
    if isinstance(payload, dict):
        ...
        if isinstance(payload.get("challenge_targets"), dict):
            return [{"id": k, "target_url": v} for k, v in payload["challenge_targets"].items()]
        return [
...
def _audit_dataset(path: Path, registry: dict[str, dict]) -> dict:
    ...
    with path.open() as f:
        for line in f:
            ...
            cid = md.get("challenge_id") or md.get("challenge") or obj.get("challenge_id") or obj.get("challenge")
            if cid:
                ids.append(cid)
            ...
            if sys_txt:
                ...
                stats["system_urls_found"] += len(re.findall(r"https?://[^\s\)\]]+", sys_txt))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write online RL readiness report JSON.")
    parser.add_argument("--run-root", required=True)
    parser.add_argument("--registry", default="/workspace/open-ctf-env/configs/challenges/cybench.yaml")
    parser.add_argument("--target-map", default="/workspace/open-ctf-env/configs/challenges/cybench_target_map_runpod.json")
    parser.add_argument("--online-rl-cybench", "--grpo-cybench", dest="online_rl_cybench", default=None)
    parser.add_argument("--online-rl-quality", "--grpo-quality", dest="online_rl_quality", default=None)
    ...
    online_rl_cybench = (
        ...
        if args.online_rl_cybench
            ...
            Path("/workspace/open-ctf-env/data/online_rl_cybench40.jsonl"),
            Path("/workspace/open-ctf-env/data/grpo_cybench40.jsonl"),
        )
    )
    online_rl_quality = (
        ...
        if args.online_rl_quality
            ...
            Path("/workspace/open-ctf-env/data/online_rl_quality.jsonl"),
            Path("/workspace/open-ctf-env/data/grpo_quality.jsonl"),
        )
    )
    ...
    report = {
        ...
        "registry_with_ground_truth_flag": sum(1 for c in registry.values() if c.get("ground_truth_flag")),
